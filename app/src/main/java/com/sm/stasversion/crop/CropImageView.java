...
package com.sm.stasversion.crop;
...
public class CropImageView extends FrameLayout {
  ...
  /** Stas */
  private Float mPostRotate = 0.0f;
  private Boolean mFirst = true;
  private Boolean mFirstCut = true;
   ...
   */
  public void getCroppedImageAsync(Bitmap image) {
    mBitmap = image;
  ...
  public float postScaleX() {
      if(mDegreesRotated == 90 || mDegreesRotated == 270) {
          return mFlipVertically ? -mPostScale : mPostScale;
      } else {
          return mFlipHorizontally ? -mPostScale : mPostScale;
      }
  }

  public float postScaleY() {
      if(mDegreesRotated == 90 || mDegreesRotated == 270) {
          return mFlipHorizontally ? -mPostScale : mPostScale;
      } else {
          return mFlipVertically ? -mPostScale : mPostScale;
  ...
  public void straightenImage(Float angel, Float scale) {
      if (mBitmap != null) {
          mFirst = false;
          mPostRotate = angel;
          mPostScale = scale;
  ...
  private void applyCustom() {
      ...
      m.postScale(postScaleX, postScaleY, mFrame.getWidth() / 2.0f, mFrame.getHeight() / 2.0f);
      m.postRotate(mPostRotate, mFrame.getWidth() / 2.0f, mFrame.getHeight() / 2.0f);
  ...
  private void applyImageMatrix(float width, float height, boolean center, boolean animate) {
    if (mBitmap != null && width > 0 && height > 0) {
      ...
      float scale;
        ...
        if(mDegreesRotated == 90 || mDegreesRotated == 270) {
            mFrame.setScaleX(scale);
            mFrame.setScaleY(scale);
        } else {
            mFrame.setScaleX(1);
            mFrame.setScaleY(1);
        }

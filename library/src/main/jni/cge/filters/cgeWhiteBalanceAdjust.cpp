...
namespace CGE
{
    ...
    void CGEWhiteBalanceFilter::setTempAndTint(float temp, float tint)
    {
        m_program.bind();
        m_program.sendUniformf(paramTemperatureName, temp * 5400.0f + 6500.0f);
        m_program.sendUniformf(paramTintName, tint);
    }

    void CGEWhiteBalanceFilter::setTint(float tint)
    {
        m_program.bind();
        m_program.sendUniformf(paramTintName, tint);
    }
